...
| `cduo init --target <path> [--home <path>]` | Initialize another project directory, optionally overriding the cduo home for isolated automation |
...
- Use `CDUO_HOME` or `cduo init --home <path>` to override the guide directory
  in tests or isolated automation.
