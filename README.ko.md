...
| `cduo doctor paths` | cduo, Claude, Codex, guide 경로 출력 |
| `cduo doctor hooks` | Claude hook 설정 파일과 command 수 출력 |
| `cduo status [--verbose]` | native foreground-session 동작 안내 |
| `cduo init` | Claude `Stop` hook을 보장하고 `AGENTS.md`와 `CLAUDE.md`에 절대 경로 `~/.cduo/orchestration-guide.md` 참조 추가 |
| `cduo init --force` | `~/.cduo/orchestration-guide.md`, 참조, `.claude/settings.local.json` 강제 갱신 |
| `cduo init --target <path>` | `ldgr` / `hrns` 자동화 흐름처럼 다른 프로젝트 디렉터리를 초기화 |
...
- `CLAUDE.md`와 `AGENTS.md`에서 cduo orchestration 참조 제거
- 기존 프로젝트 로컬 `.cduo/orchestration.md`가 있으면 제거
